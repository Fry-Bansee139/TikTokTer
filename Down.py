...
banner = f"""{war}
 ...
 /_/   /_/   /____/\___/__,__/_//_/_/\___/\_,_/\_,_/
   ...
   {M}• {P}Author {M}: {P}The WelsyDun
   {M}• {P}Judul {M}: {K}/TIKTOK/DOWNLOAD/TERMUX
   {M}• {P}Team   {M}: {H}Duta Indo Nesia Clan | Clash Of Clan!!!

 {M}• {P}Masukan Link Video Tiktok {M}({H}https://vt.tiktok.com/balablaba{M})"""

path = "/storage/emulated/0/TermuxDownloadTT"
try:
        os.mkdir(path)
except:
        pass
...
class Main:
	...
	def dow_tt(self, tk_vid, id_vid, nama_vid):
		...
		try:
			print (f' {M}• {P}Keur Ngadownload VideoTT {M}....')
			run = xyz.get(f'https://tikmate.app/download/{tk_vid}/{id_vid}.mp4?hd=1').content
			with open(f"{path}/{nm_vid}.mp4", "wb") as sv:
				sv.write(run)
				print (f'\n {M}• {P}Geus Aya Di {M}: {H}{path}/{nm_vid}.mp4')
		except KeyError:
			exit(f' {M}• {P}Url/Video Maneh Error syxtax')
	def get_dat(self, url):
		...
		try:
			...
			if 'true' in data:
				...
				print (f'\n {M}• {P}Nama Nu Boga {M}: {H}{at_vid}')
				print (f' {M}• {P}ID TTVideo {M}: {H}{id_vid}')
				print (f' {M}• {P}Tanggal Nu Upload Video {M}: {H}{tll_up_vid}')

				print (f'\n {M}• {P}Total Ngomen {M}: {H}{tl_cm_vid}')
				print (f' {M}• {P}Total Like  {M}: {H}{tl_lk_vid}')
				print (f' {M}• {P}Total Share {M}: {H}{tl_sh_vid}')
				gas = input(f'\n {M}• {P}Hayang Download Video {M}[{H}Y/T{M}]:{H} ')
				if gas in ['Y','y']:
					print (f' {M}Kamu Nanya Wkwkwkw')
					nama_vid = input(f'\n {M}• {P}Masukeun Nama Video {M}[{P}OUTPUT{M}]{M}:{H} ')
					self.dow_tt(tk_vid, id_vid, nama_vid)
				else:
					print(' {M}• {P}Makasih Geus Make Tool Aing*Wildun')
